...
const ZigZagService = memo(function ZigZagService({ title, text, image, reverse, servicePage, setCurrentPage, learnMoreLabel }) {
  ...
  return (
    <motion.div
      ...
      className={`group scroll-mt-28 grid items-stretch gap-12 rounded-none border border-transparent bg-transparent p-4 transition-all duration-500 hover:border-slate-200 hover:bg-slate-50/80 hover:shadow-[0_24px_60px_rgba(15,23,42,0.08)] sm:p-5 lg:grid-cols-2 lg:gap-16 lg:p-6 ${SOLUTION_CARDS_CLICKABLE ? "cursor-pointer" : ""}`}
    >
