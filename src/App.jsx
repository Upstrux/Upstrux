...
export default function UpstruxWebsite() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-950">
      <header className="absolute left-0 right-0 top-0 z-50 pt-7 text-white"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 md:flex-row md:items-start md:justify-between"><a href="#home" aria-label="UPSTRUX home"><Logo /></a><nav className="flex flex-wrap items-center justify-end gap-3 pt-3 text-xs font-light uppercase tracking-[0.12em] text-white md:gap-12 md:text-sm md:tracking-[0.18em]">{navItems.map((item) => item.key === "solutions" ? <div key={item.key} className="relative" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}><button className="flex items-center gap-2 transition-colors hover:text-blue-300">{item.label}<span className={`transition-transform duration-200 ${servicesOpen ? "rotate-180" : ""}`}>▼</span></button>{servicesOpen && <div className="absolute right-0 top-full mt-3 w-[92vw] max-w-[420px] border border-white/10 bg-slate-950/[0.832] p-4 shadow-2xl backdrop-blur-md md:left-1/2 md:right-auto md:w-[420px] md:aspect-[1.68/1] md:-translate-x-1/2 md:bg-slate-950/[0.168] md:p-5"><div className="flex flex-col gap-2 text-[13px] normal-case tracking-normal text-white md:h-full md:justify-between md:text-[15px]">{currentServices.map((service) => <a key={service.title} href={`#service-${slugify(service.title)}`} className="block rounded-lg px-2 py-1.5 font-light leading-5 text-slate-100 transition hover:bg-white/10 hover:text-blue-300">{service.title}</a>)}</div></div>}</div> : <a key={item.key} href={item.href} className="transition-colors hover:text-blue-300">{item.label}</a>)}<div className="flex items-center gap-2 text-xs font-light tracking-[0.12em]">{["bg", "en", "de"].map((lang) => <button key={lang} type="button" onClick={() => setLanguage(lang)} className={`cursor-pointer uppercase transition hover:text-blue-300 ${language === lang ? "text-blue-300" : "text-white"}`}>{lang}</button>)}</div></nav></div></header>
      <main>
